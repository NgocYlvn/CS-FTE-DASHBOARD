...
def parse_yvf(file_bytes):
    """
    Đọc sheet 'YVF Promotion Effectiveness'.

    Cấu trúc thực tế:
    OFFICE | Month | Total YVF Bookings | Total IFF Shipments | YVF Booking Ratio

    Lưu ý:
    Không dùng điều kiện chứa chuỗi "yvf booking" để rename vì
    "YVF Booking Ratio" cũng chứa chuỗi này và sẽ tạo cột trùng tên.
    """
    empty_cols = ["Office", "Month", "YVF Bookings", "IFF Shipments", "YVF Ratio"]
    ...
    # Map theo tên cột CHÍNH XÁC của workbook.
    normalized = {clean_text(c).casefold(): c for c in df.columns}
    ...
    rename = {}
    for normalized_name, target in exact_map.items():
        source_col = normalized.get(normalized_name)
    ...
    # Tạo các cột optional nếu workbook chưa có.
    for c in ["YVF Bookings", "IFF Shipments", "YVF Ratio"]:
        if c not in df.columns:
            df[c] = np.nan

    # Sau exact mapping, mỗi tên cột phải là Series 1-D.
    # Nếu workbook vô tình có header trùng, lấy cột đầu tiên để tránh pd.to_numeric(DataFrame).
    for c in ["YVF Bookings", "IFF Shipments", "YVF Ratio"]:
        selected = df.loc[:, c]
    ...
    # Tính lại ratio từ numerator / denominator để không phụ thuộc cached formula Excel.
